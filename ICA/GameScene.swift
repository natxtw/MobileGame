...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        //Left and Right Swipe variables
        let rightSwipe = UISwipeGestureRecognizer(target: self, action: #selector(handleSwipe(sender:)))//First swipe is the default
        let leftSwipe = UISwipeGestureRecognizer(target: self, action: #selector(handleSwipe(sender:)))
        leftSwipe.direction = .left
        
        //Up and Down Swipe variables
        let upSwipe = UISwipeGestureRecognizer(target: self, action: #selector(handleSwipe(sender:)))
        upSwipe.direction = .up
        let downSwipe = UISwipeGestureRecognizer(target: self, action: #selector(handleSwipe(sender:)))
        downSwipe.direction = .down
        
        //Adding therecognizer for all gestures
        ...
        //Setting the players physics/collisions
        ...
        scoreLabel.position = CGPoint(x: self.frame.size.width / 8 , y: self.frame.size.height / 10)
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if playerPoints < 0
        {
            ...
            self.run(SKAction.playSoundFileNamed("Loser.mp3", waitForCompletion: false)) //plays sound effect
        }
        
        if playerPoints >= 500 //upon winning, transitions into a temp winning page
         {
    ...
    {
        print ("Test to see if the monster actually hit the player")
        monster.removeFromParent()
        playerPoints -= 200
        
    }
    
    @objc func handleSwipe(sender: UISwipeGestureRecognizer) //switch statement to deteremine which case is being called
    {
        if sender.state == .ended { //after the gesture has ended
                switch sender.direction {
                case .right:
                    self.player.position = CGPoint(x: self.size.width * 0.1, y: self.size.height * 0.5)
                case .left:
                    print("Left has no currently intended functionality")
                case .up:
                    self.player.position = CGPoint(x: self.size.width * 0.1, y: self.size.height * 0.8)
                case .down:
                    self.player.position = CGPoint(x: self.size.width * 0.1, y: self.size.height * 0.2)
                default:
                    break
                }
            }
...
extension GameScene: SKPhysicsContactDelegate
{
    func didBegin(_ contact: SKPhysicsContact) {
      var firstBody: SKPhysicsBody//obj 1
      var secondBody: SKPhysicsBody//obj 2
